...
/-!
# D13: proof-carrying elimination of the finite `f ≥ 2` two-prime box

The inherited ordinary reduction gives the finite constraints used in
`finite_fge2_no_solution`.  This file checks the remaining arithmetic without
using the open conjecture and without compiler-trusted evaluation.
-/

namespace D13Round19
...
/-- The exact decimal encoding `p,e,q,f` when `1 ≤ f ≤ 9`. -/
...
def es : List ℕ :=
  (List.range 15).filter fun e ↦ 1 ≤ e

def fsHigh : List ℕ :=
  (List.range 10).filter fun f ↦ 3 ≤ f

/-- Consequences of `q^(f-1) ≤ 1120*14*f`, for `3 ≤ f ≤ 9`. -/
def qBound : ℕ → ℕ
  | 3 => 216
  | 4 => 39
  | 5 => 16
...
/-- For `f=2`, parity forces `p=2`; only `q,e` remain. -/
def casesTwo : List Case :=
  (primesUpTo 2240).flatMap fun q ↦
    es.map fun e ↦ ⟨2, q, e, 2⟩

/-- For `f≥3`, the inherited inequality gives the small `qBound f` table. -/
...
def isSolution (c : Case) : Prop :=
  encoded c.p c.q c.e c.f = c.p ^ c.e * c.q ^ c.f

def solutions : List Case := cases.filter isSolution

/-- The structural reduction leaves exactly this many kernel-checked cases. -/
theorem cases_length : cases.length = 21112 := by
  decide +kernel

/-- Kernel evaluation of the reduced finite certificate finds no arithmetic solution. -/
theorem solutions_empty : solutions = [] := by
  decide +kernel
...
theorem not_solution_of_mem {c : Case} (hc : c ∈ cases) : ¬ isSolution c := by
  intro hs
  have hm : c ∈ solutions := by
    simp [solutions, hc, hs]
  rw [solutions_empty] at hm
  simp at hm

/-- If the final one-digit exponent is `2`, parity forces the first prime to be `2`. -/
...
/-- Remove the harmless factor `p^(e-1) ≥ 1` from the inherited size inequality. -/
...
/-- A certified cutoff: if `q^r ≤ C < (B+1)^r` then `q ≤ B`. -/
...
/--
Inside the inherited finite `f≥2` box, the exact two-prime decimal encoding
cannot equal `p^e*q^f`.
-/
theorem finite_fge2_no_solution (p q e f : ℕ)
    (hp : p.Prime) (hq : q.Prime) (hpq : p < q)
    (he1 : 1 ≤ e) (he14 : e ≤ 14)
    (hf2 : 2 ≤ f) (hf9 : f ≤ 9)
    (hq2240 : q ≤ 2240)
    (hineq : p ^ (e - 1) * q ^ (f - 1) ≤ 1120 * e * f) :
    encoded p q e f ≠ p ^ e * q ^ f := by
  intro hfix
  by_cases hf : f = 2
  · subst f
    have hp2 := p_eq_two_of_f2_solution p q e hp hq hpq hfix
    subst p
    have hc2 := mem_casesTwo q e hq hq2240 he1 he14
    have hc : (⟨2, q, e, 2⟩ : Case) ∈ cases := by
      exact List.mem_append_left _ hc2
    exact (not_solution_of_mem hc) hfix
  · have hf3 : 3 ≤ f := by omega
    have hqB := q_le_qBound p q e f he14 hf3 hf9 hineq
    have hB216 := qBound_le_216 f hf3 hf9
    have hq216 : q ≤ 216 := hqB.trans hB216
    have hp216 : p ≤ 216 := by omega
    have hch := mem_casesHigh p q e f hp hq hpq hp216 hq216 hqB he1 he14 hf3 hf9
    have hc : (⟨p, q, e, f⟩ : Case) ∈ cases := by
      exact List.mem_append_right _ hch
    exact (not_solution_of_mem hc) hfix

-- Cheap semantic mutations for the arithmetic encoding itself.
example : encoded 2 3 11 2 = 21132 := by norm_num [encoded]
example : encoded 2 7 7 2 = 2772 := by norm_num [encoded]
example : encoded 2 23 3 2 = 23232 := by norm_num [encoded]
example : encoded 3 19 3 3 = 33193 := by norm_num [encoded]
example : encoded 2 3 11 2 ≠ 2 ^ 11 * 3 ^ 2 := by norm_num [encoded]

#print axioms D13Round19.cases_length
#print axioms D13Round19.solutions_empty
#print axioms D13Round19.finite_fge2_no_solution
