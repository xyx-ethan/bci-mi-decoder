...
state = {'passed': False, 'strict_build': False, 'run_id': os.getenv('GITHUB_RUN_ID'), 'commit': os.getenv('GITHUB_SHA')}
ALLOW = {'propext','Classical.choice','Quot.sound'}

def save(): (OUT/'status.json').write_text(json.dumps(state, indent=2)+'\n')
def call(args, name, timeout=2700):
...
def main():
    ...
    man=json.loads((FC/'lake-manifest.json').read_text())
    assert next(p['rev'] for p in man['packages'] if p['name']=='mathlib')==MATHLIB
    src=(ROOT/'FiniteFge2.lean').read_text()
    for bad in ['sorry','admit','native_decide','Lean.ofReduceBool','Lean.trustCompiler','unsafe','axiom ','maxRecDepth']:
        assert bad not in src,bad
    mod=FC/'D13Round19'; mod.mkdir(exist_ok=True)
    (mod/'FiniteFge2.lean').write_text(src)
    shutil.copy2(ROOT/'FiniteFge2.lean',OUT/'FiniteFge2.lean')
    lake=FC/'lakefile.toml'; txt=lake.read_text()
    old='defaultTargets = ["FormalConjectures", "FormalConjecturesUtil", "FormalConjecturesForMathlib"]'
    assert old in txt
    txt=txt.replace(old,'defaultTargets = ["D13Round19"]',1)
    txt+='\n[[lean_lib]]\nname = "D13Round19"\nglobs = ["D13Round19.+"]\n'
    lake.write_text(txt); (OUT/'lakefile.strict.toml').write_text(txt)
    call(['lake','exe','cache','get'],'cache.log',1800)
    log=call(['lake','--wfail','build'],'lake_wfail_build.log',2700)
    names=['D13Round19.cases_length','D13Round19.solutions_empty','D13Round19.finite_fge2_no_solution']
    ax={}
    for n in names:
    ...
    state.update(passed=True,strict_build=True,command='lake --wfail build',upstream_commit=PIN,
                 lean_toolchain='leanprover/lean4:v4.33.1',mathlib_revision=MATHLIB,
                 source_sha256=hashlib.sha256(src.encode()).hexdigest(),axioms_records=len(ax))
    save(); shutil.copy2(__file__,OUT/'build.py')
    (OUT/'SHA256SUMS').write_text(''.join(hashlib.sha256(p.read_bytes()).hexdigest()+'  '+p.name+'\n' for p in sorted(OUT.iterdir()) if p.is_file() and p.name!='SHA256SUMS'))
    print('D13_R19_FINITE_FGE2_STRICT_PASS',state,flush=True)
