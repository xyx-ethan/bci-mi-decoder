...
theorem q1_exponent_residue (k : ℕ)
  ...
  have h797 : 797 ∣ 740*10^k+1 := dvd_trans (by decide : 797 ∣ 2391) hdiv
  ...
  have hred : Nat.ModEq 797 (740*10^k+1) (740*10^(k%199)+1) := ((Nat.ModEq.refl 740).mul hp).add (Nat.ModEq.refl 1)
...
theorem q2_exponent_mod_six (k : ℕ)
  ...
  have hred : Nat.ModEq 7 (370*10^k+1) (370*10^(k%6)+1) := ((Nat.ModEq.refl 370).mul hp).add (Nat.ModEq.refl 1)
...
theorem q2_exponent_mod_155 (k : ℕ)
  ...
  have hred : Nat.ModEq 311 (370*10^k+1) (370*10^(k%155)+1) := ((Nat.ModEq.refl 370).mul hp).add (Nat.ModEq.refl 1)
...
theorem f_one_fixed_to_residual (p q e : ℕ)
    ...
    (∃ t, p=7 ∧ e=4 ∧ q=D13.q1 t) ∨ (∃ t, p=3 ∧ e=7 ∧ q=D13.q2 t) := by
  ...
  · rcases h with ⟨rfl,rfl⟩
    ...
    have hk : (Nat.digits 10 q).length = 136+199*t := by dsimp [t]; omega
  ...
  · rcases h with ⟨rfl,rfl⟩
    ...
    have hk : (Nat.digits 10 q).length = 666+930*t := by dsimp [t]; omega
...
theorem f_one_fixed_exists_iff_residual_prime :
    (∃ p q e : ℕ, p.Prime ∧ q.Prime ∧ p<q ∧ 0<e ∧ OeisA67599.a (p^e*q)=p^e*q) ↔
  ...
  · rintro ⟨p,q,e,hp,hq,hpq,he,hfix⟩
    ...
    · rcases h with ⟨t,rfl,rfl,rfl⟩; exact Or.inl ⟨t,hq⟩
    · rcases h with ⟨t,rfl,rfl,rfl⟩; exact Or.inr ⟨t,hq⟩
  · rintro (⟨t,ht⟩ | ⟨t,ht⟩)
